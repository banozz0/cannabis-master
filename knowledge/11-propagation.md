...
Propagation is the start of every grow. Get it wrong and you lose weeks before veg even begins; get it right and you start with healthy, traceable plants. This file covers seed germination, cuttings/clones, mother plant management, an introduction to tissue culture (full protocol in `19-tissue-culture.md`), transplanting, and hardening off. Cross-reference with `01-genetics-strains.md`, `09-disease.md` (HLVd/clean-stock risk), `19-tissue-culture.md`, `10-training.md`, `02-environment.md`, `04-medium-coco.md`, `04-medium-living-soil.md`, and `workflows/new-grow-setup.md`.

## How to Use This File

1. **Pick seed-or-clone first** — different protocols, different timelines, different risk profiles.
2. **Match propagation method to skill level + scale** — paper towel/direct sow for small beginner runs; starter cubes for repeatability; aero cloners only when you can keep them clean; tissue culture for specialist clean-stock work.
...
4. **Quarantine and consider HLVd testing for mothers/incoming clones** — especially if clones come from outside your garden or plants show unexplained dudding. Seeds are usually lower-risk than clones.
5. **Clean cutting tools** — every cut can move sap/pathogens. Prefer fresh sterile blades per plant, or clean sap off tools and use a validated disinfectant/contact time between plants.
...
| Best for | Pheno-hunting, breeding, beginners learning, fresh genetics | Repeating a known plant, preserving a keeper, uniform home runs |
| HLVd risk | Low | Very high — the #1 transmission route in licensed cannabis |

**Practical rule:** seed-started grows are generally lower-risk for HLVd than clone networks, but not automatically risk-free. Clone-heavy setups need quarantine, tool hygiene, and testing when symptoms or high-value mothers justify it.
...
- 6–12 months: continue periodic inspection and test if unexplained dudding, deformation, or reduced vigor appears
...
**Conservative HLVd testing approach for clone-based grows:**
- Quarantine incoming clones 14–21 days and inspect under magnification before introduction
- Test high-risk/high-value mothers or incoming clones through a reputable plant diagnostic lab
- If unexplained dudding, deformation, or repeated clone underperformance appears, isolate and test before taking more cuts
- Root tissue can be useful for confirmation, but follow the lab's sampling instructions

For home growers, the key is not a rigid testing calendar; it is clean sourcing, quarantine, fresh blades/tool hygiene, and not propagating from suspicious plants.
...
**What it is:** Pre-charged plug germination. Consistent, easy to transplant, and useful when starting multiple seeds.
...
**For rockwool:**
- Pre-soak in mildly acidic water, usually **pH 5.5–5.8** unless the product label says otherwise — rockwool is naturally alkaline and should be conditioned before use
...
1. **Clean-stock confidence** — known source, no unexplained dudding, and HLVd-tested if high-risk/high-value
...
- Any outside clones: HLVd/pest risk; quarantine and inspect/test before introducing
- Tissue-culture-cleaned stock: highest cost, lowest disease risk
- Friend/peer: HLVd/pest risk; quarantine and consider testing if high-risk or valuable
...
**Prep steps:**
1. **Clean sharp blade** — prefer fresh blades per plant; otherwise remove sap and disinfect with a validated protocol/contact time
...
5. **Optional: lightly wound the lower stem** only if you already have good clone hygiene; excessive scraping increases infection risk
6. **Keep cuttings hydrated** — place them into water or rooting media promptly so they do not wilt

### Rooting Hormones

**Labeled rooting gels/powders:**
- Often use IBA (indole-3-butyric acid) or similar rooting hormones
- Follow the product label and avoid contaminating the container; pour a small amount into a separate cup
- Method: briefly coat the cut end, then insert into rooting medium
...
**Practical:** a labeled rooting product is more reliable than folk recipes. Keep it clean and do not dip plant material into the original bottle.

### Rockwool / Rapid Rooter Cubes (the standard hobby method)

**Setup:**
1. Pre-soak cubes (rockwool: usually pH 5.5–5.8; peat plugs: around pH 6.0 unless label differs) — 30 min minimum
...
**Setup:**
1. Reservoir with clean pH-appropriate water; additives are optional and should be label-compatible
2. Water temp around 68–75°F; avoid warm stagnant reservoirs that favor pathogens
...
- Scales well if sanitation is consistent
...
2. Clean pH-appropriate water; change frequently rather than relying on oxidizers
...
### HLVd / Clean-Stock Checks

For home growers, use a risk-based approach:
- Initial test or quarantine for outside clones, valuable keepers, or plants with unexplained dudding
- Re-test if repeated clone batches underperform, deform, or lose vigor without an environmental cause
- Follow the diagnostic lab's sampling instructions; sample type, cost, and turnaround vary by lab
- Do not propagate from suspicious plants while waiting for results
...
**Why use it:**
- **Clean-stock recovery** — specialist meristem culture can sometimes recover clean plants from infected valuable genetics, but success is not guaranteed
- **Mass propagation** — one mother can yield thousands of identical plantlets
- **Long-term genetic backup** — specialist labs can maintain valuable genetics under controlled conditions
- **Specialist scale** — tissue culture is useful when clean-stock maintenance justifies the cost/skill
...
2. **Surface disinfection** — trained TC protocols use disinfectants to reduce surface microbes; avoid improvising chemical recipes
3. **Establishment** — explant placed on sterile basal media such as MS or DKW with appropriate plant growth regulators under a validated protocol
...
**Practical:** beginners often pick winners on yield alone. More experienced growers also track aroma, structure, resilience, cured flower quality, and lab results if available.
...
├── Repeat a known pheno → Clone from your own clean mother or a quarantined/tested source
├── Preserve/clean a valuable keeper → Consider specialist tissue culture (see 19-tissue-culture.md)
└── Rescue genetics from HLVd → Tissue culture meristem (see 09-disease.md + 19)

SEED method?
├── Beginner / 1–4 seeds → Paper towel
├── Many seeds at once → Starter cubes (Rapid Rooter or rockwool)
...
├── Many clones and strong sanitation → Aero cloner (EzClone, TurboKlone)
├── No equipment / budget → Water cloning
└── Established living soil → Direct in coco or soil

MOTHER MANAGEMENT?
├── Set up dedicated mother space → 18/6, lighter feed, clean-stock monitoring
├── Already have moms → inspect regularly; consider HLVd testing if clone-based or symptoms appear
...
2. Source clean/quarantined? HLVd-tested if high-risk or high-value?
3. Cloning environment ready (dome, substrate temp 76–80°F, RH 80–90%, low PPFD)?
4. Mother plant healthy, vigorous, sexed, and clean-stock risk assessed?
...
9. Clean/fresh cutting tools and plant-to-plant hygiene plan?
...
- **Tier 2** — Plant diagnostic labs and HLVd assay providers — testing methodology and sampling guidance
- **Tier 2** — HLVd sampling/testing guidance from plant diagnostic providers and extension-style resources
...
- HLVd / clean-stock risk — testing, transmission, mitigation → `09-disease.md`
...
- Where propagation fits in setup workflow → `workflows/new-grow-setup.md`
- Onboarding when user has no plants yet → `workflows/onboarding.md`
