...
### Pass 3 — Ambiguous routing (request photo, offer low-risk checklist only)

Tell user explicitly:

> "Without seeing the [color change / spots / curl / weirdness], I can't responsibly diagnose. The symptom you described has 4-6 possible causes that look different in person.
>
...
>
> If a photo isn't possible right now, I can give a low-risk measurement checklist, but I won't diagnose, rank causes, assign percentages, or recommend treatment until the missing data/photo is available."

If the user cannot provide a photo or material intake data, provide only low-risk triage checks clearly labeled "LOW confidence, not a diagnosis" and stop before Pass 4.

If user sends photo → exit this workflow, route to `diagnose-from-photo.md`.

### Pass 4 — Walk troubleshooting tree

Only run Passes 4-7 when confidence is MEDIUM or better: a text-unambiguous match with sufficiently complete intake, or later evidence that resolves the ambiguity.
...
### Pass 6 — Confidence statement (capped MEDIUM)

State explicitly:
- **Confidence: MEDIUM** (text-unambiguous match + complete intake)
- **Confidence: LOW** means no diagnosis. Request missing data/photos or provide only low-risk triage checks labeled "LOW confidence, not a diagnosis."
...
Show the draft entry to the user for confirmation before saving. First diagnostic logs and any durable issue entry require preview plus explicit confirmation before writing.
...
If a photo isn't possible right now, I can give a low-risk measurement checklist, but I won't diagnose until the missing data/photo is available.

[If UNAMBIGUOUS with sufficient intake:]
...
| Walking ambiguous symptom at LOW as diagnosis | Pass 3 stops before diagnosis; LOW allows only missing-data requests or low-risk triage checks |
...
- [ ] If ambiguous or intake incomplete → no diagnosis; missing data/photo requested or low-risk triage checklist only?
