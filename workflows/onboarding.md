...
First-time state population. Triggered when state files are empty/missing and the user needs ongoing personalization, grow tracking, setup planning, or when the user explicitly asks for setup. Establishes `user-profile.md` (tier, communication style, goals) and optionally `grow-context.md` (active grow setup) so future sessions have persistent context.

This workflow runs before personalized ongoing advice when state is empty (per `SKILL.md` Rule 1). For one-off questions, skip onboarding and offer light onboarding at the end.
...
6. **After explicit save confirmation, write state and summarize saved changes.** Never silently perform initial population (per `SKILL.md` Rule 6).
...
- **Advanced/experienced:** deep terms (chelated micros, photoperiod manipulation, IBL, light recipe), multi-zone home setup, technical citations or references
...
**Goals block:**
> "What's the goal here — personal supply, learning, breeding practice, preservation, or something else lawful? Any specific quality, terpene, or cultivar goals?"
...
- Whether you want every durable state update previewed in detail, or only concise previews before confirmation
...
Per `grow-context.md` Schema Rule 6 and `SKILL.md` Rule 6: never write durable state silently. Show the draft/diff, get explicit confirmation, then summarize confirmed changes at session end.
...
- State schemas: `state/user-profile.md`, `state/grow-context.md`, `state/grow-log.md`
...
- Knowledge files for tier calibration: `00-fundamentals.md` (beginner), `21-troubleshooting-tree.md` (intermediate routing), `20-commercial-ops.md` (operations hygiene)
