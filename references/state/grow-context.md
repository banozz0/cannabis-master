...
Persistent state of user's current grow setup, equipment, and active grow. The agent updates this file silently as new info appears in conversation, then summarizes changes at session end (per SKILL.md Rule 6).
...
6. **Show the diff at session summary**, not silently. User must be able to spot bad updates.
